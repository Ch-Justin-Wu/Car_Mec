 ...
 * @brief 数据包接收处理和打印,
 *  void USART1_IRQHandler(void)在stm32f1xx_it.c里,要使能串口接收中断,dma接收
 *  还有定时器中断,配置好优先级,定时器优先级比串口、DMA高,有什么bug快快提出来!!!
 *  Packet reception processing and printing,
 *  void USART1_IRQHandler(void) is in stm32f1xx_it.c, to enable the serial port to receive interrupt, DMA receive
 *  a timer interrupt, configure the priority, the timer priority is higher than the serial port and DMA,
 *  what bugs are raised quickly!!!
 * @version 1.0
 * @date 2023-10-16
 *
 * ************************************************************************
